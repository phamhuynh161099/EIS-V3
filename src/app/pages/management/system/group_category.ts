    ...
    template: `<section>
        ...
        <div class="header-title">
            <div class="flex flex-col sm:flex-row justify-between items-center mb-3 xl:mb-0">
                <h2 class="text-2xl sm:text-3xl mr-auto">{{this.valLanguage.GroupCategory[this.optionLanguage]}}<p-progress-spinner *ngIf="loading_full" class="spinner-cl pl-3" strokeWidth="5" fill="transparent" animationDuration=".5s" [style]="{ width: '20px', height: '20px', color: '#333' }" /></h2>
                ...
                <ng-template #caption>
                    <div class="flex flex-col xl:flex-row gap-3">
                        ...
                        <p-iconfield iconPosition="left" class="flex-1">
                            ...
                            <input pInputText class="w-full" type="text" (input)="onGlobalFilter($event, dtTB)" title="{{this.valLanguage.searchByGroupCategory[this.optionLanguage]}}" placeholder="{{this.valLanguage.searchByGroupCategory[this.optionLanguage]}}" />
                        </p-iconfield>
                    </div>
                </ng-template>
                    ...
                    <tr [pEditableRow]="information" (click)="onRowClick(ri)" [ngClass]="{'selected-row': selectedRow === ri, 'clickable-row' : true}">
                        ...
                        <td *ngFor="let col of columns">
                            @if (col.field === 'val1_nm') {
                                <p-cellEditor>
                                    <ng-template #input>
                                        <p-select [filter]="true" filterBy="label" optionValue="label" (onChange)="handleChangeParentEdit($event)" [options]="parent" appendTo="body" [(ngModel)]="information[col.field]" [style]="{ width: '100%' }">
                                            <ng-template let-option #item>
                                ...
                                <p-cellEditor>
                                    <ng-template #input>
                                        <p-select [filter]="true" filterBy="label" optionValue="label" [options]="typeEdit" appendTo="body" [(ngModel)]="information[col.field]" [style]="{ width: '100%' }">
                                            <ng-template let-option #item>
                                                <p-tag [value]="option.label" class="w-full !py-2" severity="secondary" />
                                            </ng-template>
                                        </p-select>
        ...
        <div class="card">
            <div class="flex justify-between items-center mb-3">
                <span class="font-semibold text-lg sm:text-2xl">{{this.valLanguage.Create_Group_Category[this.optionLanguage]}}</span>
                <p-button label="{{this.valLanguage.btn_Create_Group[this.optionLanguage]}}" [disabled]="invalidCode" variant="outlined" severity="warn" (click)="createGroup(addGroupForm)" />
            </div>
            <form
                ...
                *ngIf="addGroupForm"
                (ngSubmit)="createGroup(addGroupForm)"
            >
                <p-floatlabel>
                ...
                <p-floatlabel>
                    <p-select [filter]="true" filterBy="label" class="w-full" formControlName="inputType" [options]="typeCreate" [showClear]="true" [emptyMessage]="this.valLanguage.select_Parent_emptyMessage[this.optionLanguage]">
                        <ng-template let-option #item>
                            <p-tag [value]="option.label" severity="secondary" class="w-full !py-2" />
                        </ng-template>
                    </p-select>
                ...
                <p-floatlabel>
                    ...
                    <label for="dt_order">Order</label>
                </p-floatlabel>
                <p-floatlabel>
...
export class GroupCategory implements OnInit, OnDestroy {
    ...
    loadDataTableCustom(event: any, param: any) {
        this.loading = true;
        this.sv.getGroupCategory(param).subscribe(
            (response) => {
    ...
    filterChooseSearch(): void {
        ...
        this.sv.getGroupSearchCategory(this.param).subscribe(
            (response) => {
                if (response !== null && response !== undefined) {
                    let dtTableRes = response.rows;
                    this.dataTable = dtTableRes.map((ele: any) => {
    ...
    createGroup(form: FormGroup) {
        if (form.valid) {
            ...
            this.sv.insertGroupCate(paramAdd).subscribe(
                (response) => {
                    if (response !== null && response !== undefined) {
    ...
    updateGroup(rowTable: any) {
        ...
        this.sv.updateGroupCate(paramAdd).subscribe(
            (response) => {
                if (response !== null && response !== undefined) {
                    if (response.indexOf('error') >= 0 || response.indexOf('Error') >= 0) {
    ...
    delGroupCate(rowTable: any) {
        const paramAdd = {
            mt_cd: rowTable.mt_cd ? rowTable.mt_cd : '',
            cdid: rowTable.cdid ? rowTable.cdid : ''
        };
        this.sv.delGroupCate(paramAdd).subscribe(
            (response) => {
                if (response !== null && response !== undefined) {
                    if (response.indexOf('error') >= 0 || response.indexOf('Error') >= 0) {

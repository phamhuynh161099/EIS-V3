...
@Component({
    ...
    standalone: true,
    templateUrl: './menu_management.html',
    styles: [`
...
export class MenuManagementSystem implements OnInit {
    ...
    reloadPage() {
        ...
        this.cols = [
            ...
            { field: 'url_link', header: 'URL', type: 'Text' },
    ...
    onRowEditInit(rowTable: any) {
        this.clonedTables[rowTable.mnno as string] = { ...rowTable };
    }

    onRowEditSave(rowTable: any) {
        delete this.clonedTables[rowTable.mnno as string];
        this.data = this.data.map((item: any) => item.mnno == rowTable.mnno ? { ...item, mn_full: 'HOME > ' + rowTable.mn_nm, chg_dt_ymd: new Date()} : item)
        this.sv.updateMenuManagement(rowTable).subscribe({
            next: () => {
    ...
    onRowEditCancel(rowTable: any, index: number) {
        this.data[index] = this.clonedTables[rowTable.mnno as string];
        delete this.clonedTables[rowTable.mnno as string];
    }
    ...
    private loadData(): void {
        ...
        this.sv.getMenuManagement().subscribe({
            next: (data) => {
                this.data = data.rows;
    ...
    onCreate(){
        this.loadingTable = true;
        this.sv.insertMenuManagement(this.selectedMenuNm, this.selectedURL).subscribe({
            next: (response) => {
                const match = response.match(/'([^']*)'/);
                if (match) {
                    const result = match[1];
                    this.sv.getMenuMgtOne(result).subscribe({
                        next: (newItem) => {
                            this.data = [
                                newItem,
                                ...this.data,
                ...
                this.selectedMenuNm = this.selectedURL = '';

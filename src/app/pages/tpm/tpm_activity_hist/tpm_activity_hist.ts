...
import { TPMActivityHistService } from './tpm_activity_hist.service';
interface Column {
    field: string;
    header: string;
    ...
    template: `
        ...
        <div class="header-title">
            <div class="flex flex-col sm:flex-row justify-between items-center mb-3 xl:mb-0">
                <h2 class="text-2xl sm:text-3xl mr-auto">{{this.valLanguage.TPMActivityHistory[this.optionLanguage]}}</h2>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 rounded justify-around gap-5 *:space-y-7 *:mt-2 box p-5 header-border-filter">
                <div>
                    <p-floatlabel class="max-w-sm mx-auto">
                        <input inputId="as_no_label" type="text" pInputText [(ngModel)]="selectedAs_no" class="w-full" />
                    ...
                    <p-floatlabel class="max-w-sm mx-auto">
                        <input inputId="as_no_label" type="text" pInputText [(ngModel)]="selectedWorker" class="w-full" />
                ...
                <div>
                    <p-floatlabel class="max-w-sm mx-auto">
                        <input inputId="machine_nm_label" type="text" pInputText [(ngModel)]="selectedMachineName" class="w-full" />
                ...
                <div>
                    ...
                    <div class="flex *:flex-1 max-w-sm mx-auto gap-3">
                        <p-floatlabel class="max-w-sm mx-auto">
                        ...
                        <p-floatlabel class="max-w-sm mx-auto">
                            <p-select inputId="finish_label" [options]="finishes" [(ngModel)]="selectedFinish" [loading]="loading" optionValue="value" [filter]="true" filterBy="label" [showClear]="true" class="w-full">
                                <ng-template let-option #item>
                                    <div class="flex items-center gap-2 w-full">
                                        <p-tag [value]="option.label" class="w-full !py-2" [severity]="option.label == 'Yes' ? 'success' : 'danger'" />
                                    </div>
                                </ng-template>
                            </p-select>
                            <label for="finish_label">{{this.valLanguage.grid_Finish[this.optionLanguage]}}</label>
                        </p-floatlabel>
                    </div>
                    <p-floatlabel class="max-w-sm mx-auto">
        ...
        <section class="card-2 p-[5px] md:p-[20px] flex-1]">
            <p-table #dt1 [scrollable]="true" scrollHeight="600px" virtualScrollItemSize="600" [loading]="loadingTable" [columns]="cols"
                [value]="data" [paginator]="true" [rows]="50" [showCurrentPageReport]="true" exportFilename="TPM Activity History" [exportHeader]="'customExportHeader'"
                ...
                <ng-template #header pTemplate="header">
                    <tr>
                        ...
                        <th *ngFor="let header of cols" [pSortableColumn]="header.field" class="sortable-header cursor-pointer">
                            <div class="header-content capitalize">
...
export class TPMActivityHistPage implements OnInit {
    ...
    selectedAs_no: string | null = null;
    ...
    constructor(private sv: TPMActivityHistService, public global_sv: GlobalsService) {
    ...
    reloadPage() {
        ...
        this.cols = [
            ...
            { field: 'mf_nm', header: this.valLanguage.grid_Manufacturer_Name[this.optionLanguage] },
            { field: 'md_cd', header: this.valLanguage.grid_Model_Code[this.optionLanguage] },
            { field: 'sp_nm', header: this.valLanguage.grid_Supplier_Name[this.optionLanguage] },
    ...
    exportToExcel() {
        ...
        if (data.length > 0) {
            this.global_sv.exportExcel(data, 'TPM Activity History','data');
        }
    }
